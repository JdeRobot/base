...
/* return -1 if point falls outside the canvas */
{
  ...
  xi = (point.x * introrob_odometrico[3] - point.y * introrob_odometrico[4] + 
	introrob_odometrico[0])*introrob_escala;
  yi = (point.x * introrob_odometrico[4] + point.y * introrob_odometrico[3] + 
	introrob_odometrico[1])*introrob_escala;
 ...
 */
{
  if (out!=NULL && myencoders!=NULL){
    (*out).x = in.x*robot[ROBOT_COS] + in.y*robot[ROBOT_SIN] -
      robot[ROBOT_X]*robot[ROBOT_COS] -
      robot[ROBOT_Y]*robot[ROBOT_SIN];
    (*out).y = in.y*robot[ROBOT_COS] - in.x*robot[ROBOT_SIN] +
      robot[ROBOT_X]*robot[ROBOT_SIN] -
      robot[ROBOT_Y]*robot[ROBOT_COS];
    return 0;
  }
  return 1;
}
...
void introrob_stop()
{
  ...
  JDESchema_set_state(introrob_schema,slept);

  if (mycolorA!=NULL){
    introrob_schema->children[(*(int *)myimport("colorA","id"))]=FALSE;
    colorAstop();
    mycolorA = NULL;
  }
  if (mylaser!=NULL)
...
void introrob_run(int father, int *brothers, arbitration fn)
{
  ...
  for(i=0;i<MAX_SCHEMAS;i++) introrob_schema->children[i]=FALSE;

  introrob_schema->father=father;
  if (brothers!=NULL)
    {
  ...
  JDESchema_set_state(introrob_schema,notready);
  ...
  printf("introrob: on\n");
  pthread_cond_signal(&(introrob_schema->condition));
  pthread_mutex_unlock(&(introrob_schema->mymutex));
}
  ...
  for(;;)
    {
      ...
      if (introrob_schema->state==slept) 
	{
	  v=0; w=0;
	  pthread_cond_wait(&(introrob_schema->condition),&(introrob_schema->mymutex));
	  pthread_mutex_unlock(&(introrob_schema->mymutex));
	}
      else 
	{
	  /* check preconditions. For now, preconditions are always satisfied*/
	  if (JDESchema_get_state(introrob_schema)==notready) JDESchema_set_state(introrob_schema,ready);
	  else if (JDESchema_get_state(introrob_schema)==ready)	  /* check brothers and arbitrate. For now this is the only winner */
	    { 
	      JDESchema_set_state(introrob_schema,winner);
	      v=0; w=0; 
	  ...
	  if (JDESchema_get_state(introrob_schema)==winner)
...
void introrob_terminate()
{
  if (fd_introrobgui!=NULL)
    {
      if (introrob_schema->guistate==on){
        introrob_hide();
        introrob_schema->guistate=off;
      }
    }
  introrob_stop();
...
void introrob_init(char *configfile)
{
  ...
  JDESchema_set_state(introrob_schema,slept);
  pthread_create(&(introrob_schema->mythread),NULL,introrob_thread,NULL);
...
void introrob_guidisplay()
{
  ...
  if ((introrob_trackrobot==TRUE)&&
    ...
    {
      introrob_odometrico[0]=-myencoders_robot[ROBOT_X];
      introrob_odometrico[1]=-myencoders_robot[ROBOT_Y];
      introrob_visual_refresh = TRUE;
    }
  ...
  if ((introrob_display_state&DISPLAY_LASER)!=0){
    ...
    if (mylaser_laser!=NULL){
      for(i=0;i<NUM_LASER;i++)
	{
	  laser2xy(i,mylaser_laser[i],&aa, myencoders_robot);
  ...
  if ((introrob_display_state&DISPLAY_ROBOT)!=0){
    ...
    /* relleno los nuevos */
    us2xy(15,0.,0.,&aa, myencoders_robot);
    xy2introrobcanvas(aa,&introrob_ego[0]);
    us2xy(3,0.,0.,&aa, myencoders_robot);
    xy2introrobcanvas(aa,&introrob_ego[1]);
    us2xy(4,0.,0.,&aa, myencoders_robot);
    xy2introrobcanvas(aa,&introrob_ego[2]);
    us2xy(8,0.,0.,&aa, myencoders_robot);
    xy2introrobcanvas(aa,&introrob_ego[3]);
    us2xy(15,0.,0.,&aa, myencoders_robot);
    xy2introrobcanvas(aa,&introrob_ego[EGOMAX-1]);
    for(i=0;i<NUM_SONARS;i++)
      {
	us2xy((15+i)%NUM_SONARS,0.,0.,&aa, myencoders_robot); /* Da en el Tvoxel aa las coordenadas del sensor, pues es distancia 0 */
...
{  
  ...
  if(introrob_canvas_mouse_button_pressed==1){
      ...
      if (introrob_state==teleoperated)
	{
	  ...
	  diff_w=myencoders_robot[ROBOT_THETA]-acos_value;
