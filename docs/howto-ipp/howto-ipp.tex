...
Cuando finalice la descarga se extrae el contenido del archivo comprimido (tgz). Se nos creará un directorio llamado \textit{l\_ipp\_ia32\_p\_5.3.XXX}, dentro del cual existe un archivo de instalación llamado \textit{install.sh}. Le otorgaremos permisos de ejecución en caso de no tenerlos y lo ejecutamos.\\
...
\begin{tabular}{|p{14cm}|}
	...
	\begin{verbatim}
===============================================================================
Welcome to the Intel(R) Integrated Performance Primitives v5.3.2.068 for Linux* on IA-32 Intel(R) Architecture Installation
...
Would you like to install the following?
    Intel(R) IPP 5.3.2.068 for Linux* on IA-32 Intel(R) Architecture
...
\begin{tabular}{|p{14cm}|}
	...
	\begin{scriptsize}
	\begin{verbatim}
-I/opt/intel/ipp/5.3.2.068/ia32/include
	\end{verbatim}
	\end{scriptsize}
	\\
	\hline
...
Y estos ayudan a que se encuentren y enlacen en tiempo de ejecución las bibliotecas dinámicas que implementan el código. El directorio de ubicacion de las librerias puede variar dependiendo de la version de las librerias de IPP\\

\begin{tabular}{|p{14cm}|}
		\hline
	\begin{scriptsize}
	\begin{verbatim}
-L/opt/intel/ipp/5.3.2.068/ia32/lib -L/opt/intel/ipp/5.3.2.068/ia32/sharedlib
...
Para poder utilizar las librerias hay que crear una variable de sistema

\begin{tabular}{|p{14cm}|}
		\hline
	\begin{scriptsize}
	\begin{verbatim}
export LD_LIBRARY_PATH=.:/usr/lib:/lib:/lib/tls/i686/cmov:/usr/local/lib:
/opt/intel/ipp/5.1/ia32/sharedlib:$HOME/jde-robot:/usr/local/share/jde-robot/:$LD_LIBRARY_PATH
	\end{verbatim}
	\end{scriptsize}
	\\
	\hline
...
\begin{itemize}
  ...
  \item Convertir una imagen con una profundidad de 1 byte  a otra de 3 bytes

	En este caso lo que realizamos es fusionar los 3 valores de cada color en una sola imagen. Al ser el valor de los 3 colores igual para cada pixel, esto nos devolvera una imagen en escala de grises con 3 bytes por pixel.
   \begin{lstlisting}[language=C]
	...
	arr[1] = (Ipp8u*)imageBN;
	arr[2] = (Ipp8u*)imageBN;
	ippiCopy_8u_P3C3R(arr, ancho, (Ipp8u*)imageSalida, ancho*3, imgtam);
  ...
  \item Multiplicar dos imágenes píxel a píxel
   ...
   \item{Como redimensionar una imagen}

   Para redimensionar una imagen se utiliza la siguiente funcion:
   ...
   \begin{lstlisting}[language=C]
	...
	dstRoi.x=0;
	dstRoi.y=0;
	dstRoi.width=320*factor;
	dstRoi.height=240*factor;
	ippiResizeSqrPixel_8u_C3R((Ipp8u*)imageOrigen, imgtam, ancho*3,
										oriRoi, (Ipp8u*)imageDestino,
										ancho*factor*3, dstRoi, factor,
										factor, 1, 1, IPPI_INTER_NN,
										pBuffer);

   \end{lstlisting}
   La variable factor es la relación entre la variable original, de forma que un valor de 0.5 divide el ancho y el largo de la imagen a la mitad y un valor de 2 aumenta el ancho y el largo al doble
...
\subsection{Cómo aplicar un filtro de bordes}

 Para realizar un filtro de bordes a una imagen se puede utilizar las siguientes función:
   ...
   \begin{lstlisting}[language=C]
	...
	ippiFilterSobelNegVertBorder_8u16s_C1R ((Ipp8u*)imageBN, ancho,
											image3X, ancho*2, roi, ippMskSize3x3,
											ippBorderConst, 0, buffer);
	ippiFilterSobelHorizBorder_8u16s_C1R ((Ipp8u*)imageBN, ancho,
											image3Y, ancho*2, roi, ippMskSize3x3,
											ippBorderConst, 0, pBuffer);
	ippiCanny_16s8u_C1R(image3X, ancho*2, image3Y, ancho*2,
								(Ipp8u*)imageResultadoBN, ancho, roi,
								low, high, buffer);
   \end{lstlisting}

 Los valores high y low se pueden cambiar para obtener distintos valores del filtro de bordes. Tanto la imagen de salida como la imagen de entrada (imageResultadoBN, imageBN) son imagenes en blanco y negro con una profundidad de un byte.
