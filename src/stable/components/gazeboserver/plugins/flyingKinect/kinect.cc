...
/////////////////////////////////////////////////
DepthCameraPlugin::DepthCameraPlugin() : SensorPlugin(),cloud((new pcl::PointCloud <pcl::PointXYZRGBA>))
{
	count = 0;
...
void DepthCameraPlugin::Load(sensors::SensorPtr _sensor,
							sdf::ElementPtr /*_sdf*/)
{
	this->parentSensor = boost::dynamic_pointer_cast<sensors::DepthCameraSensor>(_sensor);
		...
		unsigned int _depth, const std::string &_format)
{
	if ( count == 0 )
	{
		...
		pthread_t thr_kinect;
		pthread_create(&thr_kinect, NULL, &mainKinect, (void*)this);
...
/*
	...
	if ( count == 25 )
	{
		this->valuePrev = _image[0];
		math::Pose position = Sharer::getInstance()->getGzModel()->GetWorldPose();
		position.pos.y += this->inc;
		this->inc *= (-1.);
		Sharer::getInstance()->getGzModel()->SetWorldPose(position);
		struct timeval t;
		gettimeofday(&t, NULL);
		this->timePoseCh = t.tv_sec*1000000 + t.tv_usec;
		count++;
	}

	if ( (_image[0] != this->valuePrev) && (this->counter < this->iterations) )
	{
		struct timeval t;
		gettimeofday(&t, NULL);
		this->timeDpthCh = t.tv_sec*1000000 + t.tv_usec;
		long diff = this->timeDpthCh - this->timePoseCh;
		this->totalTime += diff;
		std::cout << "Time[" << this->counter << "] = " << diff << " microseconds." << std::endl;
	...
	{
		std::cout << std::endl << "Total Time = " << this->totalTime << " microseconds.\tMean Time = " << this->totalTime/this->iterations << " microseconds." << std::endl;
		std::cerr << "FIN DE LA PRUEBA." << std::endl;
		this->counter++;
	}
*/
	cv::Mat image;
	...
	Sharer::getInstance()->setCurrentPose3d();
}
...
class CameraI: virtual public jderobot::Camera
{
	public:
				...
				: prefix(propertyPrefix), cameraI(camera)
		{
			...
			cameraDescription->name = "camera RGB flyingKinect";
...
class CameraII: virtual public jderobot::Camera
{
	public:
				...
				: prefix(propertyPrefix), cameraI(camera)
		{
			...
			cameraDescription->name = "camera DEPTH flyingKinect";
			mFormat = format;
