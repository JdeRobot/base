...
namespace hokuyo{
	HokuyoManager::HokuyoManager(std::string deviceId, double min_ang, double max_ang, int clustering, int timeout)
	{
	...
	HokuyoManager::laserScan2LaserData(hokuyo::LaserScan scan, jderobot::LaserData *data)
	{
	    ...
	    for(int i = 0 ; i < data->numLaser; i++){
			if (std::numeric_limits<float>::infinity() == scan.ranges[i]){
	        	...
	        	//data->distanceData[i] = scan.ranges[data->numLaser-1-i]*1000;
