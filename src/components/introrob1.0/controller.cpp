...
namespace introrob {
	const float Controller::V_MOTOR = 0.01; // mm/s 0.05
	const float Controller::W_MOTOR = 0.05; // deg/s 0.2

	Controller::Controller(jderobot::MotorsPrx mprx, jderobot::LaserPrx lprx, jderobot::EncodersPrx eprx ) {
	...
	void Controller::stopW(){
	    this->mprx->setW(0.0);
	}
	void Controller::stopV(){
	    this->mprx->setV(0.0);
	}
	
	void Controller::goLeft (float v) {
		this->mprx->setW (v);
	}

	void Controller::goRight (float v) {
		this->mprx->setW (-v);
	}

	void Controller::goUp (float v) {
		this->mprx->setV (v);
	}

	void Controller::goDown (float v) {
		this->mprx->setV (-v);
	}
