...
namespace introrob {
  ...
  void View::display(const colorspaces::Image& image1, const colorspaces::Image& image2) {
        ...
        controller->getLaser(&distanceData);
        this->getLaser();
...
/*
	...
	void View::getEncoders () {
		...
		if(this->controller->ed->robottheta>0){
		    this->robottheta = this->controller->ed->robottheta; //Alex
	    }else{
	...
	void View::getLaser () {
		...
		for (k = 0; k < this->controller->ld->numLaser; k++) {
			...
			//std::cout << "-> " << k  << ": "  <<this->controller->ld->distanceData[k] << std::endl;
			this->numLasers ++;
		}
		laser->setLaserData(distanceData);
		//std::cout << "-> " << 0 << ": "  <<this->controller->ld->distanceData[0] << std::endl;
