...
namespace visornect {
	visornectgui::visornectgui(visornectController::PTMotorsController* ptmc_in, visornectController::LedsController* lc_in, std::string path, std::string path_rgb, std::string path_ir): gtkmain(0,0) {	
	...
	refXml = Gnome::Glade::Xml::create("./visornectgui.glade");
	
	/*Create controller*/
	if (ptmc_in != NULL){
		ptmGui = new visornectGuiModules::ptmotorsGui(ptmc_in, refXml);
	}
	if (lc_in){
		lGui = new visornectGuiModules::ledsGui(lc_in,refXml);
	}
	...
	w_event_rgb->signal_button_press_event().connect(sigc::mem_fun(this,&visornectgui::on_clicked_event_rgb));
	w_event_depth->signal_button_press_event().connect(sigc::mem_fun(this,&visornectgui::on_clicked_event_depth));
	w_reconstruct->signal_toggled().connect(sigc::mem_fun(this,&visornectgui::on_reconstruct_depth));
	w_camera_pos->signal_toggled().connect(sigc::mem_fun(this,&visornectgui::add_camera_position));
	w_lines_rgb->signal_toggled().connect(sigc::mem_fun(this,&visornectgui::on_w_lines_rgb_toggled));
	w_lines_depth->signal_toggled().connect(sigc::mem_fun(this,&visornectgui::on_w_lines_depth_toggled));
	w_view_controller->signal_activate().connect(sigc::mem_fun(this,&visornectgui::on_w_view_controller_activate));
	w_radio_depth->signal_toggled().connect(sigc::mem_fun(this,&visornectgui::on_w_radio_depth_activate));
	w_radio_rgb->signal_toggled().connect(sigc::mem_fun(this,&visornectgui::on_w_radio_rgb_activate));
	w_button_clear_lines->signal_clicked().connect(sigc::mem_fun(this,&visornectgui::on_clicked_clear_lines));

		// Mundo OpenGL
    refXml->get_widget_derived("gl_world",world);

	std::cout << "Creating Progeos Virtual Cameras" << std::endl;
	mypro= new visornect::myprogeo();
	mypro->load_cam((char*)path_rgb.c_str(),0);

	mypro->load_cam((char*)path_ir.c_str(),1);
	util = new visornect::util3d(mypro);
...
bool visornectgui::on_clicked_event_rgb(GdkEventButton* event){
	int x,y;
...
bool visornectgui::on_clicked_event_depth(GdkEventButton* event){
	int x,y;
	float xp,yp,zp,camx,camy,camz;
	float xu,yu,zu; 
	float k;
...
void visornectgui::add_cameras_position(){
	int x,y;
	float xp,yp,zp,camx,camy,camz;
	float xu,yu,zu; 
	float k;
...
void 
visornectgui::on_reconstruct_depth(){
...
void 
visornectgui::add_depth_points(const colorspaces::Image& imageDEPTH, const colorspaces::Image& imageRGB){
	float distance;
	...
	for( unsigned int i = 0 ; i < 640*480 ; i++) {
		if (((int)imageDEPTH.data[3*i+0]==255) && ( ((int)imageDEPTH.data[3*i+1] >0 ) && ((int)imageDEPTH.data[3*i+1] < 255)) && (((int)imageDEPTH.data[3*i+2] > 0 ) && ((int)imageDEPTH.data[3*i+2] < 255))){
			...
			mypro->mybackproject(i % 640, i / 640, &xp, &yp, &zp, &camx, &camy, &camz,0);
...
void 
visornectgui::add_camera_position(){
	...
	if (w_camera_pos->get_active()){
		...
		mypro->mybackproject(0,640,&c2x,&c2y,&c2z,&camx, &camy, &camz,1);
		mypro->mybackproject(480,0,&c3x,&c3y,&c3z,&camx, &camy, &camz,1);
		mypro->mybackproject(480,640,&c4x,&c4y,&c4z,&camx, &camy, &camz,1);
...
void 
visornectgui::on_w_lines_rgb_toggled(){
...
void 
visornectgui::on_w_lines_depth_toggled(){
...
void
visornectgui::on_w_view_controller_activate(){
	w_window_controller->show();
}

void
visornectgui::on_w_radio_depth_activate(){
	if (w_radio_depth->get_active())
		world->draw_kinect_with_color=false;
}

void
visornectgui::on_w_radio_rgb_activate(){
	if (w_radio_rgb->get_active())
		world->draw_kinect_with_color=true;
}

void
visornectgui::on_clicked_clear_lines(){
