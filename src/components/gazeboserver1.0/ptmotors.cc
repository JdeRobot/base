...
namespace gazebo {
    ...
    //void *ptmotorsICE(void* v)
    
    GZ_REGISTER_MODEL_PLUGIN(PTMotors)
            
    PTMotors::PTMotors(){
        
        pthread_mutex_init(&mutex, NULL);
        pthread_mutex_init(&mutexPTMotors, NULL);
        count = 0;
        std::cout << "constructor PTMotors" << std::endl;        
        
    }
            
    void PTMotors::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf){
        ...
        this->updateConnection = event::Events::ConnectWorldUpdateStart(
                boost::bind(&PTMotors::OnUpdate, this));
